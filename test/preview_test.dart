...
void main() {
  ...
  group('Preview', () {
    ...
    test('Get preview by path', () async {
      expect(
        await client.preview.getPreviewByPath(imageName, 64, 64),
        isNotNull,
      );
    });
    test('Get preview by id', () async {
      expect(
        await client.preview.getPreviewById(previewFile.id, 64, 64),
        isNotNull,
      );
    });
    test('Get preview stream by path', () async {
      expect(
        await client.preview.getPreviewStreamByPath(imageName, 64, 64),
        isNotNull,
      );
    });
    test('Get preview stream by id', () async {
      expect(
        await client.preview.getPreviewStreamById(previewFile.id, 64, 64),
        isNotNull,
      );
    });
    ...
    test('Get thumbnail stream', () async {
      expect(
        await client.preview.getThumbnailStream(imageName, 64, 64),
        isNotNull,
      );
    });
  });
